                    ...
                    <div class="card-body">
                       <br> Your articles:
                            ...
                            @foreach(App\Sub::where('user_id',Auth::id())->get() as $sub)
                                <li>
                                    <a href="/findAuth/{{$sub->auth_id}}"><strong style="font-size: 40px">{{App\User::find($sub->author_id)->name}}</strong></a>
                                    <form method="POST" action="{{url('/news/editSub')}}">
                                        @csrf
                                        <input type="hidden" name="key" value="0">
                                        <input type="hidden" name="author_id" id="author_id" value="{{$sub->author_id}}">
